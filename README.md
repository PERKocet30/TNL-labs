...
## Deploy settings that actually matter

**Persistence (do this or you lose everything).** Accounts, posts, rep, and uploaded images are all stored on disk. Hosts like Railway/Render wipe the filesystem on every redeploy unless you attach a volume.

On Railway: Settings → **Volumes** → mount at `/app/data`, then set:

```
TNL_DATA = /app/data
```

That single variable puts the database *and* the uploads folder on the persistent disk. On boot the server logs `[db] using /app/data/tnl.db` — if it says anything else, your data is temporary.

**Email verification.** Real sending uses [Resend](https://resend.com) over plain HTTP (no SMTP library, nothing to install). Free tier is 3,000 emails/month.

```
RESEND_API_KEY = re_xxxxxxxxxxxx
MAIL_FROM      = TNL LABS <noreply@tnllabs.com>   # domain must be verified in Resend
PUBLIC_URL     = https://your-app.up.railway.app  # used to build the verify link
```

If `RESEND_API_KEY` is missing the app does **not** pretend to send mail — it logs the verification link to the server console and shows an "Open link" button in the app, so you can still test the flow. Add the key when you're ready for real signups.

## Email verification flow

New accounts start unverified. They can look around, but **cannot post, react, share, collab, upload, or publish** until they confirm. A banner with a Resend button sits at the top until they do.

Existing accounts (from before this feature) are **grandfathered as verified automatically** — an update should never lock out current members.

## Publishing a portfolio

A profile is private to the network by default. Hitting **Publish portfolio** gives it a public URL — `/u/username` — that anyone can open with no account: bio, links, stats, and every piece of work, server-rendered so it previews properly when shared. Publishing requires a verified email, so anything public traces back to a real person. Unpublish at any time and the page 404s.
